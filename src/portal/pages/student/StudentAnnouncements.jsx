...
export default function StudentAnnouncements() {
  ...
  const announcements = allAnnouncements.filter(a => a.targetRoles.includes('student'))
    ...
    <div style={{ fontFamily: 'Inter, sans-serif' }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .ann-card.unread { border-left: 4px solid #2B6CB0; }
      ...
      {filtered.length === 0 && <div style={{ textAlign: 'center', color: '#A0AEC0', padding: '3rem' }}>No announcements found.</div>}
      {filtered.map(a => {
        const isRead = currentUser ? a.isRead.includes(currentUser.id) : false
        return (
          <div key={a.id} className={`ann-card ${isRead ? 'read' : 'unread'}`} onClick={() => markAnnouncementRead(a.id, currentUser?.id)}>
            ...
            {!isRead && <div style={{ fontSize: '0.7rem', color: '#2B6CB0', fontWeight: 600, marginTop: '0.75rem' }}>New</div>}
          </div>
        )
      })}
    </div>
